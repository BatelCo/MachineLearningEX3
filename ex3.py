# batel cohen 208521195
# natalie balulu 312495567
...
def train_one_epoch(x, y, params):
    ...
    for example, label in zip(x, y):
        example = np.divide(example, data_max_val)
...
def back_prop(ff_cache):
    x, y_real, z1, h1, z_output, h_output, loss = [ff_cache[key] for key in
                                                   ('x', 'y_real', 'z1', 'h1', 'z_output', 'h_output', 'loss')]
...
def print_all(train_x, train_y, params, validation_x, validation_y):
...
if __name__ == "__main__":
    ...
    alldata = np.append(data_x, data_y, axis=1)
    np.random.seed(0)
    alldata = np.random.permutation(alldata)
    data_x = alldata[:, :-1]
    data_y = alldata[:, -1]
    ...
    # train the model:
    print_all(train_x, train_y, params, validation_x, validation_y)
